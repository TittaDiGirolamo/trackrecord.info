...
We care about sustained impact. The larger and more structural an initiative is, the more carefully its long-run effect on the North Star must be articulated.
...
|------------|------|---------------------------------------------|--------------------|-----------------|-----------|---------------------------|--------|
| Phase 0 — Measurement Validation & Freeze | Medium | Reliable measurement is a necessary condition for any subsequent claim about demand. Frozen counting rule: REQUIREMENTS_Phase0_Measurement.md v1.0.0 §4.1 — `profile_viewed` then `prediction_detail_viewed` with `status=resolved` in one Plausible visit. `figure_selected` is diagnostic only. | Official funnel last step: `prediction_detail_viewed (resolved)` on funnel `Completed Accountability Lookup` (sequential, other activity allowed) | Enables a trustworthy baseline. No demand claim until Pass. | Frozen 2026-08-31 20:31 Europe/Amsterdam | Pass 2026-08-31. 14-day baseline **closed 2026-09-14**: last-step unique **2** (1 = operator T-01). profile_viewed 6, conversion 33%. Direct 17 + AI 7. Google/X/GitHub **0**. Raw ~1 lookup/week; operator-adjusted ~0.5/week. Not a demand claim. Log: `audits/2026-09-02-baseline-readout.md`. | Pass |
| 14-day post-freeze baseline | Tiny | A frozen site plus a frozen metric produces a number we can compare later work against. | Official funnel last step | A weekly last-step unique count, not a growth target | Closed 2026-09-14 | **2** last-step uniques in 14 days. Discovery mix: Direct + Grok only. | Shipped |
| Phase 1 — live-path friction check | Tiny | If search → profile → resolved detail still hides a required block from REQUIREMENTS_Profile_Surface.md or REQUIREMENTS_Prediction_Detail_UI.md, the 6→2 baseline conversion is partly a product defect. If the trail is already visible on a phone, friction is not the dominant hole. No new requirements. | `profile_viewed` → resolved `prediction_detail_viewed` | Either one concrete required-block defect fixed, or Phase 1 closed as not the bottleneck | 2026-09-21 | 2026-09-21 phone walk (operator). Search “Sutton” showed Chris Sutton suggestion. Profile + pred-2026-06-10-sutton-japan-group: quote, original source, criteria, evidence block, False status, trail control all visible. Quote is not itself a link (original source is). Operator reports primary evidence URL opened FIFA knockout view, not group standings — evidence-target note, not a missing UI block. No required-block defect. Friction is not the dominant bottleneck. Operator session; not demand. | Shipped |
| Phase 2 — controlled visibility (actions 1–6) | Medium | Reaching people already interested in prediction accuracy or public accountability will produce completed lookups and reactions that can be read against the decision matrix. Cap 6 discrete factual actions. No CTA beyond “the record is public.” | Official funnel last step | At most a small move vs baseline (~0.5–1/week) plus qualitative notes; not a demand claim from one action | Action 1: 2026-09-21 | **1/6** 2026-09-21 ~20:02 Europe/Amsterdam. Own X account @titta_girolamo, no Sutton tag. Post: https://x.com/titta_girolamo/status/2102096024957268221 — detail URL only (direct detail landings do not count unless the Sutton profile is also opened in the same visit). Same venue locked until 2026-10-05. Next action must be a different permitted channel, or X after that date. Result cell to be filled 2026-09-28 from Plausible last-step unique vs baseline. Do not treat action 1 engagement as demand. | In progress |
