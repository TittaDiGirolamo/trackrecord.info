# Trackrecord.info — Main Extraction Prompt v1.3
...
### 2. Author Extraction (Very Important)

**Always try to extract the author** from the text or context.

- If the author name is clearly mentioned (e.g. "by Dan Edwards", "Dan Edwards writes", "according to Andrew Beasley"), extract it into:
  - `author.lastname`
  - `author.firstname`
- If only the full name is available, split it logically (e.g. "Dan Edwards" → lastname: "Edwards", firstname: "Dan").
- If no author name can be found in the text or byline, set both to `null`.
- Do **not** guess or invent author names.
