# Trackrecord.info — Main Extraction Prompt v1.8
...
### 2. Author Extraction (Primary Rule + Limited Fallback)

**Primary Rule:**
- First, try to extract the author directly from the text or byline.
- If a clear author name is present, extract it into `lastname` and `firstname`.

**Limited Fallback (Only when no name is found):**
- If no author name can be found in the text after careful reading, you may **lightly infer** the author from the domain or well-known context **only** in these cases:
  - The domain is strongly associated with a single well-known author (e.g., `natesilver.net` → Nate Silver).
  - The inference is highly confident and commonly known.
- In all other cases, set `author` to `null`.
- **Never guess** obscure or ambiguous author names.

**Examples:**
- `natesilver.net` and no name in text → `{"lastname": "Silver", "firstname": "Nate"}`
- `theanalyst.com` and no name in text → `{"lastname": null, "firstname": null}` (do not guess)
- No domain clue and no name → `{"lastname": null, "firstname": null}`
